...
PermissionsController = function(app, mongoose, config) {
    ...
    app.get(app.v1 + '/perms/:id', app.ensureAuthenticated, function show(req, res, next) {
    ...
    app.del(app.v1 + '/perms/:id', app.ensureAuthenticated, function destroy(req, res, next) {
        Permission.findById(req.params.id, function(err, perm) {
            checkErr(
                next,
                [{ cond: err }, { cond: !perm, err: new NotFound('json') }],
    ...
    app.put(app.v1 + '/perms/:id', app.ensureAuthenticated, function update(req, res, next) {
        Permission.findById(req.params.id, function(err, perm) {
            checkErr(
                next,
                [{ cond: err }, { cond: !perm, err: new NotFound('json') }],
    ...
    app.post(app.v1 + '/perms', app.ensureAuthenticated, function create(req, res, next) {
